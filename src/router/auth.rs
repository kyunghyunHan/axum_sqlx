...
pub async fn join(input: Json<JoinUser>) -> JsonResponse<Value> {
    ...
    let query =
        sqlx::query("SELECT id, user_id FROM users WHERE user_id = $1 AND user_pw = $2")
            .bind(&input.user_id) // user_id 값 바인딩
            .fetch_all(&connection)
            .await
            .expect("Failed to fetch users.");
...
pub async fn secession(input: Json<JoinUser>) -> JsonResponse<Value> {
    let connection: sqlx::Pool<sqlx::Postgres> = connection().await;

    let query =
        sqlx::query("SELECT user_id FROM users WHERE user_id = $1 AND user_pw = $2")
            .bind(&input.user_id) // user_id 값 바인딩
            .fetch_all(&connection)
            .await
            .expect("Failed to fetch users.");
